function [T_filter,filter, filterSD, parameters]= particle_filter_reduced_model_continuous_time(TY, Y, M, delta)
...
for iteration=1 : floor(size(TY,2)/resampling_period)
    ...
    %sampling
    parfor j=1:M
         [K,XF1,TXF1]=modified_next_reaction_method_limit_model(k(:,j),v(:,j),resampling_period*dTY);
         v(:,j)=XF1(:,size(XF1,2));
         k(:,j)=K;
